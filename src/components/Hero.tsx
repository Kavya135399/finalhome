...
import { Search, MapPin, ChevronDown, Sparkles, ShieldCheck, Gift } from 'lucide-react';
...
export function Hero() {
  ...
  return (
    <section className="bg-white dark:bg-slate-900 px-4 pt-4 pb-6 select-none shrink-0 relative">
        ...
        <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar scroll-smooth">
          {categories.slice(0, 8).map((cat) => (
            <button
              ...
              className="flex flex-col items-center gap-1.5 shrink-0 active-scale"
            >
              <div className="w-14 h-14 rounded-2xl bg-brand-50 dark:bg-slate-850 flex items-center justify-center shadow-soft border border-brand-100/30">
                {/* Generate fallback colors based on category slug */}
                <div className="w-7 h-7 rounded-lg bg-brand-600 flex items-center justify-center text-white text-xs font-black">
                  {cat.name.slice(0, 2).toUpperCase()}
                </div>
              </div>
              <span className="text-[10px] font-bold text-gray-700 dark:text-gray-300 max-w-[64px] text-center truncate">
                {cat.name}
              </span>
            </button>
          ))}
